...
@section('content')
    <div class="h-screen bg-white flex flex-col space-y-10 justify-center items-center">
        <div class="bg-white w-96 shadow-xl rounded p-5">
            ...
            <form action="{{route('admin.payment.process')}}" class="space-y-5 mt-5" method="POST">
                @csrf
                <input name="amount" type="number" step="0.01"
                       class="w-full h-12 border border-gray-800 rounded px-3 @error('amount') border-red-500 @enderror"
                       placeholder="сумма платежа"/>
                @error('amount')
                <p class="text-red-500">{{$message}}</p>
                @enderror

                <input name="sender" type="number"
                       class="w-full h-12 border border-gray-800 rounded px-3 @error('sender') border-red-500 @enderror"
                       placeholder="введите счет пользователя"/>
                @error('sender')
                <p class="text-red-500">{{$message}}</p>
                @enderror
